...
import backend.online.gamebanana.GBModDataResult;
import backend.online.gamebanana.GBModSubmitter;
...
import components.dialogs.GBModInfoDialog;

using StringTools;

class GBModComponent extends VBox {
    ...
    public function new(mod:GBMod, idx:Int = 0) {
        super();
        this.mod = mod;
        var res:GBModDataResult = GB.getModData(mod);
        if (res.error != ''){
            var dialog = new ScrollDialog('Error collecting mod data for mod ${mod._sName}!', res.error, true);
            dialog.showDialog();
            return;
        } else {
            modData = res.modData;
        ...
        onClick = (_) -> {
            var dialog = new GBModInfoDialog(mod, modData);
            dialog.showDialog();
        }
        ...
        modName.text = Misc.truncateToFitWidth(mod._sName, infoBox.width, 20);
        ...
        Update.register(this, (_) -> {
            ...
            if (mainBox != null)
                mainBox.width = infoBox.width;
            modName.text = Misc.truncateToFitWidth(mod._sName.trim(), width, 20);
        });

        haxe.Timer.delay(() -> {
            loadImage('${mod._aPreviewMedia._aImages[0]._sBaseUrl}/${mod._aPreviewMedia._aImages[0]._sFile}');
        }, 100 + (idx * 100));
    }
    ...
    public function loadModData() {
        ...
        data.text = '${((mod._aRootCategory._sName))}\n${modData.downloads} Downloads • ${modData.likes} Like${((modData.likes == 1) ? '' : 's')}';
    ...
    public function loadAuthor(author:GBModSubmitter) {
        BitmapData.loadFromFile(author._sAvatarUrl).onComplete((bitmapData:BitmapData) -> {
            authorBox = new HBox();
            ...
            authorName.text = author._sName.trim();
            ...
            authorBox.horizontalAlign = 'left';
            var authorName:Label = new Label();
            authorName.text = 'Submitted by ${author._sName.trim()}';
            authorName.verticalAlign = 'center';
            authorName.horizontalAlign = 'left';
            authorBox.addComponent(authorName);
            mainBox.addComponent(authorBox);
